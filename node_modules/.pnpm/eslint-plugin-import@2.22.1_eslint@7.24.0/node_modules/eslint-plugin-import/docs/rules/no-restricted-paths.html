...
<h2>Rule Details</h2><p>This rule has one option. The option is an object containing the definition of all restricted <code>zones</code> and the optional <code>basePath</code> which is used to resolve relative paths within.
The default value for <code>basePath</code> is the current working directory.
Each zone consists of the <code>target</code> path and a <code>from</code> path. The <code>target</code> is the path where the restricted imports should be applied. The <code>from</code> path defines the folder that is not allowed to be used in an import. An optional <code>except</code> may be defined for a zone, allowing exception paths that would otherwise violate the related <code>from</code>. Note that <code>except</code> is relative to <code>from</code> and cannot backtrack to a parent directory.
You may also specify an optional <code>message</code> for a zone, which will be displayed in case of the rule violation.</p>
...
<pre><code class="language-js">import bar from &#39;../server/bar&#39;;
</code></pre>
...
<pre><code class="language-js">import baz from &#39;../client/baz&#39;;
</code></pre>
<hr>
...
<pre><code class="language-js">import a from &#39;../two/a&#39;
</code></pre>
